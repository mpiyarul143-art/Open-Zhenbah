...
```bash
npm run dev
# open https://open-fiesta.vercel.app
```
...
- **OAuth redirect URL** (if using Google/GitHub sign-in):
  - Add to each provider in Supabase Auth Settings:
    ...
    - `https://open-fiesta.vercel.app/auth/callback` (Local)
...
2. **Configure OAuth providers (optional but recommended)**
   - In Supabase Dashboard -> Authentication -> Providers:
     ...
     - Set Redirect URLs:
       - Local: `https://open-fiesta.vercel.app/auth/callback`
...
5. **Run the app**
   - `npm run dev` -> open https://open-fiesta.vercel.app
